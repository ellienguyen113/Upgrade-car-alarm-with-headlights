...
The ignition subsystem was modified to allow multiple start attempts, 
keep the engine running even if seatbelts are unfastened or passengers exit, 
...
| Enable ignition only when both seats are occupied                            | Turned ON all seat and seatbelt slide switches                | Red ignition-enabled LED turned on                             |    and both seatbelts are fastened
